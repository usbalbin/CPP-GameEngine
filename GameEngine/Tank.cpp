...
void Tank::setupTurret(glm::vec3 position, float yaw, float pitch, float roll, Cube* body, glm::vec3 bodyHalfExtents) {
	...
	cannon = new Barrel(renderer, physics, position + turretBasePos + cannonPos, cannonHalfExtents, cannonMass, projectileRadius, projectileMass, yaw, pitch + PI_HALF, roll, 0.1s, { FireMode::FULL }, "content/TankShot.wav");
