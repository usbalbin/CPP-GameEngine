...
Ray ray transformRay(float16 matrix, Ray ray){
	Ray result;
	result.position = mulMatVec(matrix, (float4)(ray.position, 1.0f));
	result.direction = mulMatVec(matrix, (float4)(ray.direction, 0.0f));
	result.inverseDirection = 1.0f / direction;
}
